...
(define task-panel-layout-tests
  (test-suite
   ...
   (test-case "测试列表名过长时的布局" 
     ;; 创建唯一的临时数据库文件
     (define temp-db-path (format "./temp-test-task-panel-~a.db" (current-inexact-milliseconds)))
   ...
   (test-case "测试不同视图下的布局" 
     ;; 创建唯一的临时数据库文件
     (define temp-db-path (format "./temp-test-task-panel-views-~a.db" (current-inexact-milliseconds)))
