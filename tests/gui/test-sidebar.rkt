...
(define sidebar-tests
  (test-suite
   ...
   (test-case "测试列表刷新功能"
              ;; 创建唯一的临时数据库文件
              (define temp-db-path (format "./tests/temp-test-sidebar-~a.db" (current-inexact-milliseconds)))
   ...
   (test-case "测试列表管理功能"
              ;; 创建唯一的临时数据库文件
              (define temp-db-path (format "./tests/temp-test-sidebar-lists-~a.db" (current-inexact-milliseconds)))
   ...
   (test-case "测试智能列表选中状态切换"
              ;; 创建唯一的临时数据库文件
              (define temp-db-path (format "./tests/temp-test-sidebar-selected-~a.db" (current-inexact-milliseconds)))
              ...
              (check-equal? (send today-btn get-label) (lang:translate "今天"))
              ...
              (check-equal? (send sidebar get-current-selected-original-label) (lang:translate "今天"))
              (check-equal? (send today-btn get-label) (string-append "→ " (lang:translate "今天")))
              ...
              (check-equal? (send sidebar get-current-selected-original-label) (lang:translate "计划"))
              (check-equal? (send planned-btn get-label) (string-append "→ " (lang:translate "计划")))
              (check-equal? (send today-btn get-label) (lang:translate "今天"))
   ...
   (test-case "测试自定义列表选中状态切换"
              ;; 创建唯一的临时数据库文件
              (define temp-db-path (format "./tests/temp-test-sidebar-custom-selected-~a.db" (current-inexact-milliseconds)))
   ...
   (test-case "测试多个列表之间的选中状态切换"
              ;; 创建唯一的临时数据库文件
              (define temp-db-path (format "./tests/temp-test-sidebar-multi-selected-~a.db" (current-inexact-milliseconds)))
   ...
   (test-case "测试set-selected-button方法的参数传递"
              ...
              ;; 定义一个模拟的自定义按钮类
              (define mock-custom-button%
                (class button%
                  (super-new)
                  (define/public (set-selected! val) (void))
                  (define/public (refresh) (void))))

              ;; 创建一个测试按钮
              (define test-btn (new mock-custom-button% [parent frame] [label "Test Button"]))
   ...
   (test-case "测试语言切换时智能列表按钮选中状态"
              ;; 创建唯一的临时数据库文件
              (define temp-db-path (format "./tests/temp-test-sidebar-language-~a.db" (current-inexact-milliseconds)))
