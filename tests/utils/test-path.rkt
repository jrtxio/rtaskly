...
(define path-tests
  (test-suite
   ...
   (test-case "测试默认数据库路径获取" 
     ...
     (check-equal? (file-name-from-path default-db-path) (string->path "tasks.db"))
     ;; 直接比较路径，确保比较一致
     (check-equal? (normalize-path (path-only default-db-path)) 
                   (normalize-path (get-default-app-dir))))
  
   ;; 测试目录存在性检查和创建
   (test-case "测试目录存在性检查和创建" 
     ;; 创建临时目录路径
     (define temp-dir-path (format "./tests/temp-test-dir-~a" (current-inexact-milliseconds)))
   ...
   (test-case "测试 safe-file-exists? 函数" 
     ;; 创建临时文件
     (define temp-file-path (format "./tests/temp-test-file-~a.txt" (current-inexact-milliseconds)))
